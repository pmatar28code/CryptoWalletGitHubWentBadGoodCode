...
class MainActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        ...
        if(uri != null){
            ...
            var coinBaseClient = retrofit.create(CoinBaseClient::class.java)
            var accessTokenCall = coinBaseClient.getToken(
                "authorization_code",
                code,
                MY_CLIENT_ID,
                CLIENT_SECRET,
            ...
            accessTokenCall.enqueue(object: Callback<AccessToken> {
                override fun onResponse(call: Call<AccessToken>, response: Response<AccessToken>) {
                    Toast.makeText(this@MainActivity,"good response: ${response.isSuccessful}",Toast.LENGTH_SHORT).show()
